...
import {featuredAccounts} from "@/data/home";
import {formatPrice} from "@/utils";

export default function FeaturedAccount() {
  return (
    <section className="mb-12">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold text-gray-800">Tài khoản nổi bật</h2>
        <Link href="/tai-khoan" className="text-blue-600 hover:text-blue-800 font-medium">
          Xem tất cả
        </Link>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6">
        {featuredAccounts.map((account) => (
          <Link href={`/tai-khoan/${account.id}`} key={account.id}
            ...
            <div className="p-4">
              ...
              <div className="flex items-center space-x-2">
                <div className="text-red-600 font-bold">{formatPrice(account.price)}</div>
                {account.oldPrice && (
                  <div className="text-gray-500 text-sm line-through">{formatPrice(account.oldPrice)}</div>
                )}
              </div>
