...
export async function generateMetadata({ params }: Props) {
  ...
  const accountReq = await fetch(
    `${process.env.NEXT_PUBLIC_FRONTEND_HOST}/api/accounts/${slug}`,
    {
      cache: "force-cache"
    }
  );
  ...
  return (
    <main className="container mx-auto py-6 px-4 md:px-6">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <AccountDetailSlide imageGallery={account.image ? [{ id: 0, image: account.image}, ...account.imageGallery] : account.imageGallery} />
        <AccountInfo {...account} />
      </div>
